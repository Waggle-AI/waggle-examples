...
def extract_surface_request(user_message: str) -> SurfaceRequest:
    ...
    if not api_key:
        raise ExtractionError("OPENAI_API_KEY or OPENAI_SECRET_NAME is required for this example.")
...
def _openai_api_key() -> str:
    ...
    secret_name = os.environ.get("OPENAI_SECRET_NAME", "").strip()
    if not secret_name:
        return ""
    ...
    try:
        ...
        raise ExtractionError(f"Unable to read OpenAI secret '{secret_name}': {exc}") from exc
